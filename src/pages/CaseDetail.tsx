    ...
    <AppLayout>
      <div className="flex">
        <div className="flex-1 min-w-0 p-6 max-w-4xl">
          ...
          <motion.article initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div>
              ...
              <h1 className="font-display text-2xl font-bold text-foreground mt-1">{caseItem.title}</h1>
            ...
            <div className="flex flex-wrap gap-2">
              {caseItem.tags.map((tag) => (
                <Link key={tag} to={`/?tag=${tag}`} className="px-3 py-1 rounded-full bg-secondary text-secondary-foreground text-xs hover:bg-primary/10 hover:text-primary transition-colors">
                  {tag}
                </Link>
              ))}
            </div>
            ...
            <div className="card-base p-6 space-y-4">
              <h2 className="font-display font-semibold text-lg text-foreground">概述</h2>
              <p className="text-sm text-card-foreground leading-relaxed">{caseItem.summary}</p>
              <h2 className="font-display font-semibold text-lg text-foreground">背景与挑战</h2>
              <p className="text-sm text-card-foreground leading-relaxed">
                在快速变化的业务环境中，团队面临着多方面的挑战。传统方法已无法满足日益增长的业务需求，需要创新性地解决核心痛点。本案例详细记录了从问题发现到方案落地的完整过程。
              </p>
              <h2 className="font-display font-semibold text-lg text-foreground">解决方案</h2>
              <p className="text-sm text-card-foreground leading-relaxed">
                经过深入调研和方案比选，团队最终选择了分阶段推进的策略。第一阶段聚焦基础能力建设，第二阶段进行场景化应用，第三阶段实现规模化推广。每个阶段都设置了明确的里程碑和验收标准。
              </p>
              <h2 className="font-display font-semibold text-lg text-foreground">成果与反思</h2>
              <p className="text-sm text-card-foreground leading-relaxed">
            ...
            <div className="flex items-center gap-4 py-4 border-t border-b border-border">
              <button className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-secondary text-secondary-foreground text-sm hover:bg-primary/10 hover:text-primary transition-colors">
            ...
            {/* Comments area */}
            <div>
              <h3 className="font-display font-semibold text-foreground mb-4">参与讨论</h3>
        ...
        <aside className="w-72 shrink-0 border-l border-border p-5 hidden xl:block sticky top-0 h-screen overflow-y-auto">
          <h3 className="font-display font-semibold text-sm text-foreground mb-4">相关推荐</h3>
