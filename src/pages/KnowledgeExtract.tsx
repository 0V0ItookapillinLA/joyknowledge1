...
const KnowledgeExtract = () => {
  ...
  if (appMode === "quick-template") {
    return (
      <AppLayout>
        <div className="flex flex-col h-[calc(100vh-56px)] bg-background">
          {/* Header with actions unified at top */}
          <div className="px-8 pt-4 pb-3 border-b border-border">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  className="flex items-center gap-2 px-5 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-all shadow-sm disabled:opacity-50">
                  <Wand2 className="w-4 h-4" /> 确认生成
                </motion.button>
              </div>
            </div>
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <h1 className="text-lg font-bold text-foreground mb-0.5">选择输出模板</h1>
              <p className="text-xs text-muted-foreground">选择一个常用模板，AI 将基于你的 {sources.length} 份资料生成对应格式的知识文档</p>
            </motion.div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="max-w-5xl mx-auto px-8 py-4">
              {/* Source summary bar */}
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-4 p-2.5 rounded-lg border border-border bg-card">
                <Layers className="w-4 h-4 text-primary shrink-0" />
                <span className="text-xs text-foreground font-medium">已上传 {sources.length} 份资料</span>
                ...
                {OUTPUT_TEMPLATES.map((tpl, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${tpl.gradient} flex items-center justify-center shrink-0`}>
                          <tpl.icon className="w-3.5 h-3.5 text-white" />
                        </div>
                        <h3 className="text-xs font-semibold text-foreground">{tpl.label}</h3>
                      </div>
                      <p className="text-[10px] text-muted-foreground leading-relaxed mb-2">{tpl.desc}</p>
                      <div className={`rounded-lg p-2 transition-colors ${isSelected ? tpl.color : "bg-accent/50"}`}>
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                          {tpl.preview.map((item, j) => (
                            <div key={j} className="flex items-center gap-1">
                              <div className={`w-1 h-1 rounded-full ${isSelected ? "bg-primary" : "bg-muted-foreground/30"}`} />
                              <span className="text-[10px] text-muted-foreground whitespace-nowrap">{item}</span>
                            </div>
                          ))}
                        </div>
                      </div>
